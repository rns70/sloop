...
const ADR_REFINEMENT_PROTOCOL = [
  ...
  '       command. A `verify:` MUST be a real runnable command (e.g. `npm test`, `pytest -q`),',
  '       never the restated criterion text or prose — a non-command fails forever with',
  '       "command not found". If nothing can check it via a shell, omit verify (manual QA).',
  '       Push back on vague or unverifiable criteria.',
...
const SYSTEM = [
  ...
  '',
  'Keep the ADR tree connected. A new ADR almost always belongs under an existing one. Before',
  'calling create_adr, use list_docs / read_doc to find the right parent, then pass its relPath (or',
  'id) as `parent` — this nests the new ADR beside the parent and registers it in the parent\'s',
  'children automatically (do not hand-edit the parent for this). Only omit `parent` when the ADR is',
  'genuinely top-level. Never leave a new ADR orphaned from the structure.',
  '',
