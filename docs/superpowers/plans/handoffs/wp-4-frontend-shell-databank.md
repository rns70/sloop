...
4. `MarkdownEditor` (in `src/web/design/`) — wrap **BlockNote** (`@blocknote/core`, `@blocknote/react`, `@blocknote/mantine`), a block-based rich-text editor, NOT a textarea:
   ...
   - **Expose integration hooks for WP-7 (author assistant)** so WP-7 never edits this component: an `onSelectionChange?(selectedText: string)` callback, and an imperative `applyProposal(originalText: string, replacement: string)` (or a `proposal` prop) that shows the change as an inline diff pending accept/reject. Keep these minimal but present.
